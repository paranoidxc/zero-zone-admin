...
export const useSettingsStore = defineStore("settings", () => {
  ...
  function getTabsList() {
    const unique = (arrs) => {
      const res = new Map();
      return arrs.filter(
        (arr) => !res.has(arr.meta.fullPath) && res.set(arr.meta.fullPath, 1),
      );
    };
  ...
  function activeTabs(router) {
    ...
    if (
      tabsList.value.filter((e) => e.meta.fullPath === router.meta.fullPath)
        .length === 0
    ) {
      router.meta.isActive = true;
      curRouter.value = router;
      tabsList.value.push(router);
    }
  }

  return { isShowMenu, update, tabsList, getTabsList, activeTabs, curRouter };
});
